...
export async function fetchGmailThreads(accessToken: string, userEmail: string): Promise<ParsedJobThread[]> {
  ...
  const query = 'from:me (internship OR application OR recruiter OR hiring OR role OR opportunity OR position OR career OR job OR referral OR interview OR applied OR apply) newer_than:30d';
